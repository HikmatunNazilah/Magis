...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Only allow editing own logbooks that are pending or ditolak
    ...
    if ($row['status_validasi'] === 'disetujui') {
        echo json_encode(['success' => false, 'message' => 'Logbook yang sudah disetujui tidak dapat diedit.']);
        exit();
    }

    $sql = "UPDATE Logbook SET tanggal='$tanggal', kegiatan='$kegiatan', status_validasi='pending' WHERE id_logbook=$id_logbook AND mahasiswa_id=$mahasiswa_id";
    if ($conn->query($sql)) {
